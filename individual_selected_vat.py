...
def select_sids():
    ...
    selected_indices = input("Enter the indices of the subject IDs you want to analyze (separated by commas): ")
    selected_indices = [int(idx) - 1 for idx in selected_indices.split(",")]
    selected_sids = [sids[idx] for idx in selected_indices]
    return selected_sids

selected_sids = select_sids()
dall_summ = proc(selected_sids, 'angle')

# 绘制VAT數據
fig_vat, ax_vat = plt.subplots(figsize=(10, 6))
for key, grp in dall_summ.groupby(['condition']):
    ax_vat = grp.plot(ax=ax_vat, kind='line', x='errblock', y='error', yerr='se', label=key)

ax_vat.set_xlabel('Error Block')
ax_vat.set_ylabel('Error (mean ± SE)')
ax_vat.set_title('VAT Data')
ax_vat.legend(title='Condition')
plt.show()
...
DCCS = [101, 100, 88, 95, 123, 85, 121, 115, 86, 98, 109, 93, 90, 132, 93, 94, 101, 121, 81, 104, 116, 89] #median=99
Flanker = [109, 106, 118, 104, 118, 113, 108, 123, 128, 111, 106, 109, 96, 133, 106, 102, 102, 125, 67, 136, 115, 101] #median=109
Visual_reasoning = [105, 108, 114, 106, 109, 103, 128, 127, 109, 106, 105, 109, 109, 119, 111, 105, 77, 101, 96, 124, 109, 85] #median=108.5
